...
object NaiveBayes {
  ...
  def countAttributeValues(data:List[Map[String, Any]], attribList:String): Map[Any,Int]= {
    val attrib = data.flatMap(x => x.get(attribList))
    attrib.groupBy( x => x ).mapValues(_.size)
  }
  ...
  def calcPriorPropabilities(data:List[Map[String, Any]], classAttrib:String):Map[Any,Double]= {
    val values = countAttributeValues(data, classAttrib)
    values.map {
      case (x, v) => (x, v.toDouble / data.size)
    }
  }
  ...
  def calcAttribValuesForEachClass(data:List[Map[String, Any]], classAttrib:String):
    ...
    data
      .groupBy(_.get(classAttrib))
      .transform((x,y) => y
        .flatMap(z => z.groupBy(_._1))
        .groupBy(_._1).transform((x,y) => y
        .map(_._2))).transform((x,y) => y
      .transform((x,y)=> y.flatten.groupBy(_._2)
        .mapValues(_.size)).filter(_._1 != classAttrib).toSet)
  ...
  Map[Any,Set[(String, Map[Any, Double])]] = {
    ...
    data.transform((z, y) => y.map(x => (x._1, x._2.mapValues(x => x.toDouble/ classCounts(z)))))
  ...
  def calcClassValuesForPrediction(record:Map[String,Any], conditionalProps: Map[Any,Set[(String, Map[Any, Double])]],
    ...
    priorProps.map(x => (x._1 -> x._2 * record.flatMap(y => conditionalProps(x._1.toString).filter(z => (z._1 == y._1)).map(t => t._2.getOrElse(y._2, 0.0))).product))
  }
  ...
  def findBestFittingClass(classProps:Map[Any,Double]):Any= {
    classProps.toList.reduce((x,y) => if(x._2 > y._2) x else y)._1
  }
  ...
  Map[Any,Set[(String, Map[Any, Double])]] = {
    ...
    data.transform((z, y) => y.map(x => (x._1, attValues(x._1).zipWithIndex.toMap.transform((a, b) => (x._2.getOrElse(a, 0).asInstanceOf[Double] + 1.0) /(classCounts(z) + attValues(x._1).size)))))
